/**
 * Deploy anonymous-clinical-trial-platform contract to a Midnight network.
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy Clinical Trial Contract to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`\n  📍 Wallet Address (${network}): ${address}\n`);

  console.log('  Syncing with network...');
  const syncStart = Date.now();
  const syncTimeoutMs = 120_000; // 2 minute sync timeout log hint
  let timedOut = false;

  const syncInterval = setInterval(() => {
    const elapsed = Math.round((Date.now() - syncStart) / 1000);
    process.stdout.write(`\r  ⏳ Still syncing... (${elapsed}s elapsed)   `);
    if (elapsed > 120 && !timedOut) {
      timedOut = true;
      console.log(`\n  ⚠ Network sync taking longer than expected (${elapsed}s).`);
      console.log('  If Preprod wallet sync is blocked by indexer status, local deployment remains available.');
    }
  }, 5000);
  ...
  console.log('  Initializing Trial Protocol (ID: 101)...');
  try {
    const initTx = await (deployed as any).callTx.initializeTrial(101n);
    console.log(`  Trial Initialized! TxId: ${initTx.public.txId}\n`);
  } catch (initErr) {
    console.log('  Note: Trial initialization can also be called via CLI.\n');
  }
